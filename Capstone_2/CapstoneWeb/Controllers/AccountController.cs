...
namespace CapstoneWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
       {
        ...
        [AllowAnonymous]
        public ActionResult ExistingRegister(int Id,string UserName)
        {
            userBAL = new UserBAL();
            Session["ExistingTAMCEApplicationId"] = Id;

            LicenseBAL objBAL = new LicenseBAL();
            ApplicationModel model = objBAL.GetApplication(Id, Status.All, "Transaction");
            ViewBag.ApplicantName = model.APMCEModel.RegistrationModel.Name;
            ViewBag.HouseNumber = model.APMCEModel.RegistrationModel.HouseNumber;
            ViewBag.StreetName = model.APMCEModel.RegistrationModel.StreetName;
            ViewBag.DistrictList = userBAL.GetCountries();

            return View("ExistingRegister");
        }
        ...
        public string GetExistingLicenseDetails(string LicenseNumber)
        {
            
            userBAL = new UserBAL();
            DataTable objExistingLicense  = userBAL.GetExistingLicenseDetails(LicenseNumber);
            if (objExistingLicense.Rows.Count > 0)
                Session["existingLicenseEmailId"] = objExistingLicense.Rows[0]["Email"].ToString();
            return  JsonConvert.SerializeObject(objExistingLicense);

        }

        public ActionResult ExistingApplication(int Id)
        {
            APMCEBAL apmceBAL = new APMCEBAL();
            UserBAL objBAL = new UserBAL();
            ViewBag.DistrictList = objBAL.GetCountries();

            LicenseQuestionnaireModel model = new LicenseQuestionnaireModel();

            model.ApplicationModel = objBAL.GetApplication(Id, Status.All);
            if (model.ApplicationModel.ExistingPCPNDTModel != null)
            {
                model.ApplicationModel.ExistingPCPNDTModel.EquipmentList = null;

                model.HasAppliedforPCPNDT = true;
                model.ApplicationModel.ExistingPCPNDTModel.ApplicationId = Id;
            }
            Session["ExistingApplicationId"] = Id;         
            ...
            if (model.ApplicationModel.ExistingPCPNDTModel != null)
            {
                Session["ExistingTransactionId"] = model.ApplicationModel.ExistingPCPNDTModel.TransactionId;
                Session["ExistingEquipmentsList"] = model.ApplicationModel.ExistingPCPNDTModel.EquipmentList;
                Session["ExistingEmployeeList"] = model.ApplicationModel.ExistingPCPNDTModel.EmployeeList;
                Session["OtherUploadsList"] = model.ApplicationModel.ExistingPCPNDTModel.DeclarationModel.OtherUploadsList;

                // Set file paths
                Session["ExistingAddressProofPath"] = model.ApplicationModel.ExistingPCPNDTModel.FacilityModel.AddressProofPath;
                Session["ExistingBuildingLayoutPath"] = model.ApplicationModel.ExistingPCPNDTModel.FacilityModel.BuildingLayoutPath;
                Session["ExistingAffidavitDocPath"] = model.ApplicationModel.ExistingPCPNDTModel.InstitutionModel.AffidavitDocPath;
                Session["ExistingArticleFilePath"] = model.ApplicationModel.ExistingPCPNDTModel.InstitutionModel.ArticleDocPath;
                Session["ExistingStudyCertificates"] = model.ApplicationModel.ExistingPCPNDTModel.InstitutionModel.StudyCertificateDocPaths;
            }
            return View("ExistingApplication", model);
        }
       
       
        public ActionResult AddEquipment(ExistingEquipmentModel model)
        {
            if (ModelState.IsValid)
            {
                HttpPostedFileBase _uploadedFile = Request.Files[0];

                int _applicationId = Session["ExistingApplicationId"] == null ? 0 : Session.GetDataFromSession<int>("ExistingApplicationId");
                int _transactionId = Session["ExistingTransactionId"] == null ? 0 : Session.GetDataFromSession<int>("ExistingTransactionId");
                ...
                List<ExistingEquipmentModel> objEquipmentsList;
                if (Session["ExistingEquipmentsList"] != null)
                    objEquipmentsList = Session.GetDataFromSession<List<ExistingEquipmentModel>>("ExistingEquipmentsList");
                //TempData["EquipmentsList"] as List<EquipmentModel>;
                else
                    objEquipmentsList = new List<ExistingEquipmentModel>();
                objEquipmentsList.Add(model);
                Session.SetDataToSession<List<ExistingEquipmentModel>>("ExistingEquipmentsList", objEquipmentsList);
        ...
        { 
            if (Session["ExistingEquipmentsList"] != null)
            {
                List<ExistingEquipmentModel> objEquipmentsList = Session.GetDataFromSession<List<ExistingEquipmentModel>>("ExistingEquipmentsList");
                //TempData["EquipmentsList"] as List<EquipmentModel>;
        ...
        public JsonResult AddEmployee(ExistingEmployeeModel model)
        {
            if (ModelState.IsValid)
            {
                HttpPostedFileBase _uploadedFile = Request.Files[0];
               ...
               // model.CreatedUserId = Session.GetDataFromSession<UserModel>("User").Id;
                ...
                List<ExistingEmployeeModel> objEmployeeList;
                if (Session["ExistingEmployeeList"] != null)
                    objEmployeeList = Session.GetDataFromSession<List<ExistingEmployeeModel>>("ExistingEmployeeList");
                // TempData["EmployeeList"] as List<EmployeeViewModel>;
                else
                    objEmployeeList = new List<ExistingEmployeeModel>();
                objEmployeeList.Add(model);
                Session.SetDataToSession<List<ExistingEmployeeModel>>("ExistingEmployeeList", objEmployeeList);
        ...
        public JsonResult DeleteEmployee(int index)
        {
            if (Session["ExistingEmployeeList"] != null)
            {
                List<ExistingEmployeeModel> objEmployeeList = Session.GetDataFromSession<List<ExistingEmployeeModel>>("ExistingEmployeeList");
        ...
        public JsonResult SaveApplicantDetails(ExistingPCPNDTViewModel model, HttpPostedFileBase AddressProof, HttpPostedFileBase BuildingLayout, HttpPostedFileBase affidavitFile, HttpPostedFileBase articleFile, HttpPostedFileBase[] studyCertificateFiles)
        {
            userBAL = new UserBAL();
            NotificationModel notification = new NotificationModel();
            int _serviceId = 2;
            ...
            {
                #region File Saving
                var uploadsPath = Path.Combine("Applicant", "Existing"
                ...
                if (AddressProof != null)
                {
                    ...
                    model.FacilityModel.AddressProofPath = _addressProofPath + Path.GetExtension(AddressProof.FileName);
                ...
                else if (Session["ExistingAddressProofPath"] != null)
                {
                    model.FacilityModel.AddressProofPath = Session.GetDataFromSession<string>("ExistingAddressProofPath");
                }
                ...
                if (BuildingLayout != null)
                {
                    ...
                    model.FacilityModel.BuildingLayoutPath = _buildingLayoutPath + Path.GetExtension(BuildingLayout.FileName);
                ...
                else if (Session["ExistingBuildingLayoutPath"] != null)
                {
                    model.FacilityModel.BuildingLayoutPath = Session.GetDataFromSession<string>("ExistingBuildingLayoutPath");
                }



                if (model.InstitutionModel.OwnershipTypeId > 0)
                {
                    ...
                    if (affidavitFile != null)
                    {
                        ...
                        model.InstitutionModel.AffidavitDocPath = _affidavitDocPath + Path.GetExtension(affidavitFile.FileName);
                    ...
                    else if (Session["ExistingAffidavitDocPath"] != null)
                    {
                        model.InstitutionModel.AffidavitDocPath = Session.GetDataFromSession<string>("ExistingAffidavitDocPath");
                    }

                    // Saving Study Certificates
                    if (studyCertificateFiles != null && studyCertificateFiles.Length > 0)
                    {
                        model.InstitutionModel.StudyCertificateDocPaths = new List<DocumentUploadModel>();
                        ...
                        foreach (var file in studyCertificateFiles)
                        {
                            ...
                            docModel.UploadedUserId =  0;
                            model.InstitutionModel.StudyCertificateDocPaths.Add(docModel);
                        ...
                        if (Session["ExistingStudyCertificates"] != null)
                            model.InstitutionModel.StudyCertificateDocPaths.AddRange(Session.GetDataFromSession<List<DocumentUploadModel>>("ExistingStudyCertificates"));
                    }
                    else
                    {
                        model.InstitutionModel.StudyCertificateDocPaths = Session.GetDataFromSession<List<DocumentUploadModel>>("ExistingStudyCertificates");
                    }

                    if (Session["ExistingEmployeeList"] != null)
                        model.EmployeeList = Session.GetDataFromSession<List<ExistingEmployeeModel>>("ExistingEmployeeList");
                    if (Session["ExistingEquipmentsList"] != null)
                        model.EquipmentList = Session.GetDataFromSession<List<ExistingEquipmentModel>>("ExistingEquipmentsList");
                    #endregion


                    bool result = userBAL.saveExistingLicense(model);
                    ...
                    if (result)
                    {
                        ...
                        notification.Title = "Success";
                        notification.NotificationType = NotificationType.Success;
                        notification.NotificationMessage = "Applicant details saved";
                        notification.ShowActionButton= true;
                        notification.ActionButtonText = "Ok";
                        notification.ControllerName = "Home";
                        ...
                        string msgBody = "Hi " + model.ApplicantModel.Name + "," + " Your UserName :  " + model.ApplicantModel.Email + " Password :  " + "123" + "  From Capstone Team(PCPNDT, APMCE, BLOODBANK). ";
                        string MailBody= "Hi " + model.ApplicantModel.Name + "," + " <br/><br/> LoginName :  " + model.ApplicantModel.Email + "<br/><br/> Password :  " + "123" + "  .<br/><br/>Thanks & Regards,<br/>Capstone Team.<br/> ";
                        String MailSubject = "Capstone Application : Your Login Credentials";

                        string deliveryStatus;
                        Utitlities.SendSMS(model.ApplicantModel.MobileNo, msgBody, out deliveryStatus);
                        Utitlities.SendEmail(model.ApplicantModel.Email, MailBody,MailSubject);
