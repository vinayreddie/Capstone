...
namespace Capstone.BAL
{
    public class MasterBAL
    {
        ...
        public List<OfferedServiceMasterModel> GetOfferedServicesByHospitalTypeId(int hospitalTypeId)
        {
            ...
            {
                // TODO: Write exception log        - Raj, 28-12-2020
                return null;
            }
        }
        ...
        public List<QualificationModel> GetQualifications(string actType)
        {
            try
            {
                masterDAL = new MasterDAL();
                DataTable dtItems = masterDAL.GetQualifications(actType);
                if (dtItems == null)
                    return null;
                List<QualificationModel> objList = new List<QualificationModel>();
        ...
        public List<HospitalTypeModel> GetHospitalTypes()
        {
            masterDAL = new MasterDAL();
            DataTable dtItems = masterDAL.GetHospitalTypes();
            if (dtItems == null)
                return null;

            List<HospitalTypeModel> hospitalTypesList = new List<HospitalTypeModel>();
            HospitalTypeModel hospitalType;
            foreach (DataRow row in dtItems.Rows)
            {
                hospitalType = new HospitalTypeModel();
                hospitalType.Id = Convert.ToInt32(row["Id"]);
                hospitalType.Name = row["Name"].ToString();
                hospitalTypesList.Add(hospitalType);
            }

            return hospitalTypesList;
        }

        public List<EquipmentTypeModel> GetEquipmentTypesList(int HospitaltypeId)
        {
            masterDAL = new MasterDAL();
            DataTable dtItems = masterDAL.GetEquipmentTypesList(HospitaltypeId);
            if (dtItems == null)
                return null;

            List<EquipmentTypeModel> equipmentTypesList = new List<EquipmentTypeModel>();
        ...
        {
            masterDAL = new MasterDAL();
            return masterDAL.SaveOfferedServicesEquipments(model);
        }

        public List<OfferedServiceEquipmentsComplexViewModel> GetOfferedServicesEquipmentsList()
        {
            masterDAL = new MasterDAL();
            DataTable dt = masterDAL.GetOfferedServicesEquipmentsList();
            if (dt == null)
                return null;
            return ConvertOfferedServicesEquipmentsToList(dt);
        }
        private List<OfferedServiceEquipmentsComplexViewModel> ConvertOfferedServicesEquipmentsToList(DataTable dt)
        {
            List<OfferedServiceEquipmentsComplexViewModel> EquipmentList = new List<OfferedServiceEquipmentsComplexViewModel>();
            foreach (DataRow row in dt.Rows)
            {
                OfferedServiceEquipmentsComplexViewModel Equipment = new OfferedServiceEquipmentsComplexViewModel();
                Equipment.Id = Convert.ToInt32(row["Id"]);
                Equipment.ServiceId = Convert.ToInt32(row["Id"]);
                Equipment.Name = row["Name"].ToString();
                Equipment.HospitalTypeId = Convert.ToInt32(row["HospitalTypeId"]);
                Equipment.HospitalType = row["HospitalType"].ToString();//
                //Equipment.OfferedService = row["OfferedService"].ToString();
                Equipment.Equipments = row["Equipments"].ToString();
                Equipment.IsActive = Convert.ToBoolean(row["IsActive"]);
                EquipmentList.Add(Equipment);
            }
            return EquipmentList;
        ...
        public OfferedServiceEquipmentsMasterModel GetOfferedServiceEquipmentsDetails(int id)
        {
            try
            {
                masterDAL = new MasterDAL();
                DataTable dtItems = masterDAL.GetOfferedServiceEquipmentsDetails(id);
                if (dtItems == null)
                    return null;

                OfferedServiceEquipmentsMasterModel equipment = new OfferedServiceEquipmentsMasterModel();
                if (dtItems.Rows.Count > 0)
                {                    
                    equipment.Id = Convert.ToInt32(dtItems.Rows[0]["Id"]);
                    equipment.Name = dtItems.Rows[0]["Name"].ToString();
                    equipment.HospitalTypeId = Convert.ToInt32(dtItems.Rows[0]["HospitalTypeId"]);
                    equipment.EquipmentIds = dtItems.Rows[0]["Equipments"].ToString();
                    equipment.IsActive = Convert.ToBoolean(dtItems.Rows[0]["IsActive"]);
                }
                return equipment;
        ...
        public List<CentresModel> GetCentresList(int distrctId)
        {
            masterDAL = new MasterDAL();
            DataTable dtItems = masterDAL.GetCentresList(distrctId);
            if (dtItems == null)
                return null;

            List<CentresModel> centresList = new List<CentresModel>();
            CentresModel centre;
            foreach (DataRow row in dtItems.Rows)
            {
                centre = new CentresModel();
                centre.Id = Convert.ToInt32(row["Id"]);
                centre.Name = row["Name"].ToString();
